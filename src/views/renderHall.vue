			...
			<div class="form-group">
				...
				<div class="form-input">
					<select v-model="book_type">
						...
						<option :value="item.bt_id" v-for="(item) in bookTypes" :key="item.bt_id">{{item.bt_name}}</option>
					</select>
				</div>
			</div>
			<div class="form-btn">
		...
		<div class="table">
			<table cellpadding="0" cellspacing="0" >
				...
				<tr v-for="(item) in book" :key="item.book_id">
					...
					<td>
						<button @click="editBook(item.book_id)">我要借阅</button>
		...
		computed:{
			time(){//时间合适
				return (param)=>{
					...
					return date.toLocaleString().substr(0,9)
				}
			},
			bookType(){
				return (param)=>{
